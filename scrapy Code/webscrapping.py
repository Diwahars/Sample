...
def scrape_flipkart(url, no_products):
    ...
    for item in data:
        ...
        product_name.append(name.get("title image" = item.find_all('img')[0]))

        if image.get("data-src img_url" = image.get('data-src'))      
	else:
            img_url = image.get("src      image_url.append(img_url)
        price1 = item.find_all("div",{"class":"pu-final font-dark-color fk-inline-block"})[0]
        price.append(price1.text.strip())
        link.append(name.get("href    product_name_final = product_name[:no_products]
